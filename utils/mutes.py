...
async def mute(bot, serverid, userid, time):
    try:
    ...
    if prerow is None:
        ...
        role = discord.utils.get(server.roles, name='Muted')
        user = discord.utils.get(server.members, id=userid)
        time2 = datetime.utcnow() + timedelta(seconds=time)
        await bot.add_roles(user, role)
        new_mute = ServerMutes(serverId=serverid, userId=userid, timeToUnmute=time2)
        session.add(new_mute)
        session.commit()
        await bot.send_message(user, f"You have been muted in {server.name} for {time} seconds")
        return True
    else:
        return False
